...
def build_auth_url(user_id: str):
    params = {
        "audience": "api.atlassian.com", "client_id": Config.JIRA_CLIENT_ID,
        "scope": " ".join(SCOPES), "redirect_uri": Config.JIRA_REDIRECT_URI,
...
def save_tokens(user_id: str, tokens: dict):
    ...
    data = {
        "user_id": user_id, "access_token": tokens.get("access_token"),
        "refresh_token": tokens.get("refresh_token"), "expires_at": expires_at
    }
    supabase.table("jira_tokens").upsert(data).execute()

def exchange_code_for_token(code: str, user_id: str):
    payload = {
        "grant_type": "authorization_code", "client_id": Config.JIRA_CLIENT_ID,
        "client_secret": Config.JIRA_CLIENT_SECRET, "code": code, "redirect_uri": Config.JIRA_REDIRECT_URI
    }
...
def refresh_jira_token(user_id: str):
    ...
    if not user_data or not user_data.get("refresh_token"): return None

    payload = {
        "grant_type": "refresh_token", "client_id": Config.JIRA_CLIENT_ID,
        "client_secret": Config.JIRA_CLIENT_SECRET, "refresh_token": user_data["refresh_token"]
    }
    res = requests.post(Config.JIRA_TOKEN_URL, json=payload)
    if res.status_code == 200:
        new_tokens = res.json()
        if "refresh_token" not in new_tokens: new_tokens["refresh_token"] = user_data["refresh_token"]
...
def get_valid_token(user_id: str):
    ...
    if time.time() > (user_data["expires_at"] - 60): return refresh_jira_token(user_id)
...
async def disconnect_jira(user_id: str):
    try:
        ...
        res = supabase.table("jira_tokens").delete().eq("user_id", user_id).execute()
